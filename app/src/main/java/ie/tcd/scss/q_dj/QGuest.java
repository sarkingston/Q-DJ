...
public class QGuest extends AppCompatActivity {

    String partyID = "";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_qguest);
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);

        //On Click of Submit Button
        //Get The Text and send it to the 'ServerComms' class in an ASYNNCtask
        new joinParty().execute(partyID);
    ...
    private class joinParty extends AsyncTask<String, Void, Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try {
                success = new ServerComms().joinParty("userID",partyID);
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        protected void onPostExecute(Boolean result) {
            if(result){
                ...
                Snackbar.make(findViewById(android.R.id.content), "Sorry! That didn't work, that party might not exist. Try again", Snackbar.LENGTH_LONG).show();
            }
        }
